...
4. 核对技能新鲜度：根 `README.md`（快速上手 / 命令一览 / 子命令计数 / 错误码列表 / 特性段落）与本技能命令示例、「参数边界检查清单」、`docs/templates/` 是否随本轮能力面同步（口径见 `AGENTS.md`「技能新鲜度」节）。
5. bump `package.json` 版本（每轮迭代一个版本）。
...
5. **收尾** — 登记 FIX、回填 `fixed_in`、置 `approved`；核对**技能新鲜度**（含根 `README.md`：快速上手 / 命令一览 / 子命令计数 / 错误码列表 / 特性段落，口径见 `AGENTS.md`「技能新鲜度」节）；`D verify`、`D lint`、bump 版本。
...
- **段落标题精确匹配**：若 schema 配了 `content_sections`，段标题必须精确（括注写进段内正文）；变体标题不被命中。
- **写路径强制校验（US-005）**：`bind --enforce` 有 lint 前置门（error 级存在则拒开）；`--no-enforce` 显式关；不给 flag **保持现值**（re-bind 不静默改开关）；enforce 包 `add` 违规 → `SCHEMA_VIOLATION` exit 1 + `error.violations` 逐条 hint + **包字节零变化**；warn 级（编号跳号等）不拦、随成功写入以 `schema_warnings` 透传；enforce 态 `add` 必须原子带全必填字段；`rm` 不拦 `ref_exists` 悬挂、`mv`/`checkout` 未接入（后续版本）。
